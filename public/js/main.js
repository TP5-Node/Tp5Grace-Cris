...
//FUNCIONES PARA MOSTRAR MODALES // estas funciones podrian ser una sola, debidamente parametrizada
const showModal = () => {
...
//FUNCIONES PARA CERRAR MODALES // Lo mismo para este caso
const closeModal = () => {
	let container = document.getElementById('backModal')
	container.classList.toggle('hide')
}
...
const postCourse = () => {
	// aunque aparentemente se hace la validación, pude crear nuevos ingresos sin poner nada.
...
//FUNCION PARA FILTER
const filterContent = () => {
	fetch('/api/resources').then((res) => res.json()).then((response) => {
		const data = response.resources
		let valueInput = document.getElementById('filterInput').value
		let filterInput = data.filter((resource) =>
			Object.keys(resource).find((prop) => resource[prop].includes(valueInput))
		)
		const list = document.getElementById('tableContain')
		list.innerHTML = ''
...
const editCourseById = (id) => {
	fetch(`/api/resources/${id}`).then((res) => res.json()).then((res) => {
		showEditModal(id)
